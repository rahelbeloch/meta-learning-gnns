...
class Maml(GraphTrainer):
    ...
    def outer_loop(self, batch, mode):
        ...
        for graphs, targets in batch:
            ...
            local_model = self.adapt_few_shot(x_support, edge_index_support, cl_mask_support, support_targets, mode)

            # Determine loss of query set
            loss, query_predictions = run_model(local_model, query_graphs, query_targets, mode, self.loss_module)
    ...
    for support_batch_idx, batch in tqdm(enumerate(test_loader), "Performing few-shot fine tuning in testing"):
        ...
        with torch.no_grad():  # No gradients for query set needed
            ...
            for query_batch_idx, test_batch in enumerate(test_loader):
                ...
                _, pred = run_model(local_model, output_weight, output_bias, graphs, targets, mode)
